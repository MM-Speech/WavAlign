...
DATASET_PATH="${3:-${REPO_ROOT}/examples/sample_rl_sft.jsonl}"
RL_TOKENS="${4:-text}"
SFT_TOKENS="${5:-all}"
INCLUDE_BOUNDARIES="${6:-true}"
RL_SPEECH_WEIGHT="${7:-0.0}"
RL_TEXT_WEIGHT="${8:-1.0}"
SFT_SPEECH_WEIGHT="${9:-1.0}"
SFT_TEXT_WEIGHT="${10:-1.0}"

VITA_AUDIO_ROOT="${VITA_AUDIO_ROOT:?Please set VITA_AUDIO_ROOT to your local VITA-Audio checkout.}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/outputs}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/vita-audio-${VARIANT}-rl-sft-masked/${TIMESTAMP}}"
...
torchrun \
  ...
  "${REPO_ROOT}/train_vita_audio_rl_sft_masked.py" \
  ...
  --rl_token_type "${RL_TOKENS}" \
  --sft_token_type "${SFT_TOKENS}" \
  --include_audio_boundaries "${INCLUDE_BOUNDARIES}" \
  --rl_speech_weight "${RL_SPEECH_WEIGHT}" \
  --rl_text_weight "${RL_TEXT_WEIGHT}" \
  --sft_speech_weight "${SFT_SPEECH_WEIGHT}" \
  --sft_text_weight "${SFT_TEXT_WEIGHT}" \
  ...
  --reward_model_type "${REWARD_MODEL_TYPE:-api}" \
  "${@:11}"
